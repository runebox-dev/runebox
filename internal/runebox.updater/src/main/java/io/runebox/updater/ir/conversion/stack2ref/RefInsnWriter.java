...
import io.runebox.updater.ir.constant.IntConstant;
import io.runebox.updater.ir.constant.NullConstant;
...
import io.runebox.updater.ir.ref.condition.*;
import io.runebox.updater.ir.ref.expr.*;
import io.runebox.updater.ir.ref.invoke.*;
import io.runebox.updater.ir.ref.stmt.*;
import io.runebox.updater.ir.stack.StackLocal;
import io.runebox.updater.ir.stack.insn.IfInsn;
...
import io.runebox.updater.ir.util.StackInsnVisitor;
...
public class RefInsnWriter extends StackInsnVisitor<Instruction, StackLocal> {
    ...
    public void visitInvokeInsn(Invoke stackInvoke) {
        ...
        Optional<StackValue> instanceOpt = stackInvoke instanceof io.runebox.updater.ir.stack.invoke.AbstractInstanceInvoke ?
        ...
        convertOrElseMerge(allPops, () -> {
            ...
            if (refInvoke instanceof io.runebox.updater.ir.ref.invoke.AbstractInstanceInvoke) {
                RWCell<Expression> instanceCell = ((io.runebox.updater.ir.ref.invoke.AbstractInstanceInvoke) refInvoke).getInstanceCell();
    ...
    private AbstractInvoke convertAbstractInvoke(Invoke stackInvoke, List<StackValue> argumentValues, Optional<StackValue> instanceOpt) {
        ...
        if (stackInvoke instanceof io.runebox.updater.ir.stack.invoke.AbstractInstanceInvoke) {
            if (stackInvoke instanceof InterfaceInvoke) {
                InterfaceInvoke interfaceInvoke = (InterfaceInvoke) stackInvoke;
                return new InvokeInterface(interfaceInvoke.getMethod(), instanceOpt.get().getValue(), arguments);
            } else if (stackInvoke instanceof VirtualInvoke) {
                VirtualInvoke virtualInvoke = (VirtualInvoke) stackInvoke;
                return new InvokeVirtual(virtualInvoke.getMethod(), instanceOpt.get().getValue(), arguments);
            } else if (stackInvoke instanceof SpecialInvoke) {
                SpecialInvoke specialInvoke = (SpecialInvoke) stackInvoke;
                return new InvokeSpecial(specialInvoke.getMethod(), instanceOpt.get().getValue(), arguments, specialInvoke.isInterface());
            } else {
                throw new AssertionError();
            }
        } else if (stackInvoke instanceof StaticInvoke) {
            StaticInvoke staticInvoke = (StaticInvoke) stackInvoke;
            return new InvokeStatic(staticInvoke.getMethod(), arguments, staticInvoke.isInterface());
        } else if (stackInvoke instanceof DynamicInvoke) {
            DynamicInvoke dynamicInvoke = (DynamicInvoke) stackInvoke;
