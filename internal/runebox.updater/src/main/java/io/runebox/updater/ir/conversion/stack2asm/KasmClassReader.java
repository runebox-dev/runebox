...
import io.runebox.updater.ir.conversion.AccessConverter;
import io.runebox.updater.ir.typeannotation.ClassTypeAnnotation;
import io.runebox.updater.ir.typeannotation.TargetType;
 ...
 */
public class KasmClassReader {
    private final Classfile classfile;

    public KasmClassReader(Classfile classfile) {
    ...
    private void visit(ClassVisitor cv) {
        ...
        String name = AsmUtil.toInternalName(classfile.getNamePath());
    ...
    private void visitModule(ClassVisitor cv) {
        classfile.getModule().ifPresent(module -> {
            ...
            if (mv != null) {
                new KasmModuleReader(module).accept(mv);
            }
        });
    }
    ...
    private void visitFields(ClassVisitor cv) {
        for (Field field : this.classfile.getFields()) {
            ...
            if (fv != null) {
                new KasmFieldReader(field).accept(fv);
            }
        }
    }
    ...
    private void visitMethods(ClassVisitor cv) {
        for (Method method : classfile.getMethods()) {
            ...
            if (mv != null) {
                new KasmMethodReader(method).accept(mv);
            }
        }
    }
}
