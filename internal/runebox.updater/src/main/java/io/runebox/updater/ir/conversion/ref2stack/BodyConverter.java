...
import io.runebox.updater.ir.stack.TryCatchBlock;
import io.runebox.updater.ir.stack.insn.GotoInsn;
import io.runebox.updater.ir.stack.insn.Instruction;
import io.runebox.updater.ir.stack.insn.StoreInsn;
import io.runebox.updater.ir.type.ObjectType;
import io.runebox.updater.ir.type.Type;
import io.runebox.updater.ir.typeannotation.LocalVariableTypeAnnotation;
import io.runebox.updater.ir.util.RefCfgGraph;
...
public class BodyConverter {
    ...
    private void convertInstructions() {
        ...
        for (io.runebox.updater.ir.ref.TryCatchBlock tryCatchBlock : refBody.getTryCatchBlocks()) {
    ...
    private void convertTryCatchBlocks() {
        ...
        refBody.getTryCatchBlocks().stream()
                .collect(Collectors.groupingBy(io.runebox.updater.ir.ref.TryCatchBlock::getHandler))
                .forEach((handler, blocksForHandler) -> {
            Instruction handlerInsn = getInstruction(handler);
            Map<RefLocal, List<io.runebox.updater.ir.ref.TryCatchBlock>> handlersByLocal = blocksForHandler.stream()
                    .collect(Collectors.groupingBy(io.runebox.updater.ir.ref.TryCatchBlock::getExceptionLocal));
            ...
            if (handlersByLocal.size() == 1 && handlerNode.getSucceeding().isEmpty()) {
                ...
                Map.Entry<RefLocal, List<io.runebox.updater.ir.ref.TryCatchBlock>> onlyEntry = handlersByLocal.entrySet().iterator().next();
                RefLocal local = onlyEntry.getKey();
                List<io.runebox.updater.ir.ref.TryCatchBlock> blocks = onlyEntry.getValue();
    ...
    private void convertTryCatchBlocks(List<io.runebox.updater.ir.ref.TryCatchBlock> blocks, StoreInsn handlerInsn) {
        for (io.runebox.updater.ir.ref.TryCatchBlock refBlock : blocks) {
            ...
            stackBody.getTryCatchBlocks().add(new TryCatchBlock(firstInsn, lastInsn, handlerInsn, exception));
        }
    }
